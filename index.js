...
const {addEmployee, updateEmployee, deleteEmployee} = require('./controllers/employees');
    ...
    inquirer
        ...
        .then((answers) => {
            ...
            if (task === 'View all employees') {
                ...
                deleteEmployee()
                .then((answers) => dal.deleteFrom(queries.deleteId, 'employees', Number(answers.deleteEmployee)))
                .then(() => askTask());
            } else if (task === 'Delete role') {
                deleteRole()
                .then((answers) => dal.deleteFrom(queries.deleteId, 'roles', Number(answers.deleteRole)))
                .then(() => askTask());
            } else if (task === 'Delete department') {
                deleteDept()
                .then((answers) => dal.deleteFrom(queries.deleteId, 'departments', Number(answers.deleteDept)))
                .then(() => askTask());
            } else {
                process.exit();
            }
